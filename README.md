...
4. Support for timeouts/delays was introduced to [CompletableFuture](https://docs.oracle.com/javase/9/docs/api/java/util/concurrent/CompletableFuture.html) only in Java 9, so still widely supported applications running on Java 8 a left out without this important functionality. Moreover, even the newly available Java 9 API still lacks important features (default values on timeouts, non-cancelling timeout) or applies questionable design decisions (like delayed executors vs delay operator on promise).

There are numerous free open-source libraries that address some of the aforementioned issues. However, none of them provides implementation of interruptible `CompletionStage` and no one adresses _all_ of the shortcomings.
